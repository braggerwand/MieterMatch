...
const TenantSelectionPage: React.FC<TenantSelectionPageProps> = ({ landlord, tenants, onConfirm, onCancel }) => {
  ...
  // Matching-Logik (Heuristisch)
  const scoredTenants = useMemo(() => {
    return tenants
      .filter(t => t.status === 'Profil aktiv')
      .map(tenant => {
        let score = 0;
        let totalCriteria = 0;

        // 1. Mietbudget (Gewichtung: Hoch)
        totalCriteria += 30;
        if (landlord.rentWarm <= tenant.maxRent) {
          score += 30;
        } else if (landlord.rentWarm <= tenant.maxRent * 1.1) {
          score += 15;
        }

        // 2. Zimmeranzahl
        totalCriteria += 20;
        if (landlord.rooms >= tenant.minRooms) {
          score += 20;
        }

        // 3. Wohnfläche
        totalCriteria += 15;
        if (landlord.sqm >= tenant.minSqm) {
          score += 15;
        }

        // 4. Lage (PLZ Abgleich oder Textsuche)
        totalCriteria += 15;
        if (tenant.desiredLocation.toLowerCase().includes(landlord.zipCode) || 
            landlord.address.toLowerCase().includes(tenant.desiredLocation.toLowerCase())) {
          score += 15;
        }

        // 5. Ausstattung (EBK, Garten, Parken)
        totalCriteria += 20;
        if (landlord.kitchenDetails.toLowerCase().includes('ja') === tenant.kitchenIncluded.toLowerCase().includes('ja')) score += 7;
        if (landlord.gardenOrBalcony.toLowerCase() !== 'nein' && tenant.gardenOrBalcony.toLowerCase() !== 'nein') score += 7;
        if (landlord.parkingDetails.toLowerCase() !== 'nein' && tenant.parkingNeeded.toLowerCase() !== 'nein') score += 6;

        const finalScore = Math.round((score / totalCriteria) * 100);
        return { tenant, score: finalScore };
      })
        ...
        <div className="space-y-6">
          {scoredTenants.length === 0 ? (
            <div className="glass-card p-12 text-center rounded-[2.5rem] border-dashed border-2 border-white/10">
              <p className="text-gray-500">Aktuell keine passenden aktiven Profile gefunden.</p>
            </div>
          ) : (
